...
export function ThemeProvider({ children }) {
  ...
  useEffect(() => {
    ...
    try {
      const savedMode = window.localStorage.getItem("ds-theme-mode");
      if (
        savedMode === "system" ||
        savedMode === "light" ||
        savedMode === "dark"
        ...
        const old = window.localStorage.getItem("ds-theme");
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem("ds-theme-mode", mode);
      if (mode === "light" || mode === "dark") {
        window.localStorage.setItem("ds-theme", mode);
      } else {
  ...
  const toggleTheme = useCallback(() => {
    setMode((m) => {
      const effective = m === "system" ? systemTheme : m;
      return effective === "dark" ? "light" : "dark";
    });
  }, [systemTheme]);

  const value = useMemo(
    () => ({ theme, mode, setMode: setThemeMode, toggleTheme }),
